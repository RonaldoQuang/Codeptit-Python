"""
Trên cổng thực hành trực tuyến của Học viện Công nghệ Bưu chính Viễn thông có danh sách sinh viên trong lớp được xếp hạng để đánh giá kết quả. 
...
if __name__ == '__main__': 
    ...
    for _ in range (int(input())): 
        ten=input() 
        dung, sub=map(int,input().split())
        mp.append((ten, dung, sub)) 
    ds=sorted(mp, key=lambda x:(-x[1],x[2],x[0])) 
    for ten, dung, sub in ds:
            print(ten,dung,sub)
