"""
Hệ thống quản lý lịch thi học kỳ cho nhiều Môn học, mỗi môn học có các (Có thông tin Mã môn học, tên môn học) Lịch thi học kỳ bao gồm nhiều thông tin gồm: Mã ca thi, Mã môn học, Ngày thi, Giờ thi, Nhóm thi. 
...
class lich:
    mon=subject
    def __init__(self, mon, ma, ngay, d, m, y, tg, gio, phut, nhom):
        self.mon=mon
        self.ma=ma
        self.ngay=ngay
        self.d=d
        ...
        self.nhom=nhom
    def kq(self):
        print(self.ma,self.mon.ma,self.mon.ten,self.ngay,self.tg,self.nhom)
...
if __name__ == '__main__':
    ...
    for i in range (1,m+1):
        if i<10:
            ...
            ma="T"+str(i)
        id, ngay, tg, nhom=input().split()
        b=ngay.split("/")
        d, m, y=b[0], b[1], b[2]
        c=tg.split(":")
        gio, phut=int(c[0]), int(c[1])
        a.append(lich(mp[id],ma,ngay,d,m,y,tg,gio,phut,nhom))
    a.sort(key=lambda x:(x.y,x.m,x.d,x.gio,x.phut,x.mon.ma))
    for i in a:
        i.kq()
